...
/** Four placement guides on /uni (REV-31), the universities' counterpart of the hiring guides strip. */
const TONES: Tone[] = ["butter", "mint", "coral", "sage"]

export function UniGuidesStrip() {
    const posts = UNI_GUIDE_SLUGS.slice(0, 4).map((slug) => ({ slug, post: BLOG_POSTS[slug] })).filter((x) => !!x.post)
    return (
        <Section
            eyebrow="Placement guides"
            title="How to run a placement season well"
            sub="Short, practical guides for placement cells and heads of department."
            action={<Link href="/uni/guides" className="inline-flex items-center gap-1.5 border-b border-neutral-300 pb-0.5 text-sm font-medium text-neutral-900 hover:border-neutral-900">All placement guides <ArrowRight className="size-3.5" /></Link>}
        >
            <CardArtStyles />
                        ...
                        <li key={slug} className="sh-reveal" style={{ ["--sh-reveal-delay" as string]: `${i * 0.07}s` }}>
                            <Link href={`/blogs/${slug}`} className={cn("group flex h-full flex-col rounded-2xl p-6 transition-transform duration-300 hover:-translate-y-1", t.surface, t.ink)}>
                                <span className="mb-5 flex h-32 items-center justify-center rounded-xl bg-white/45">
                                    <CardArt kind={(UNI_GUIDE_ART[slug] ?? "guides") as ArtKind} className="max-h-28 transition-transform duration-500 group-hover:scale-[1.05]" />
                                </span>
