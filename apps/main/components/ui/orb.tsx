...
/*
 * The Orb, from ElevenLabs UI (https://ui.elevenlabs.io/docs/components/orb), vendored
 * for Incidents narration and talks (plan/incidents INC-21). MIT License, Copyright (c)
 * 2025 Eleven Labs Inc. The only change: the noise texture is served from
 * /incidents/perlin-noise.png instead of their CDN (local files only, no hotlinks).
 */
...
/** The shader's uniforms, named, so strict index access knows each one exists (a ShipItHQ addition). */
type OrbUniforms = Record<"uAnimation" | "uColor1" | "uColor2" | "uInputVolume" | "uInverted" | "uOffsets" | "uOpacity" | "uOutputVolume" | "uPerlinTexture" | "uTime", THREE.IUniform>
...
function Scene({
  ...
  useEffect(() => {
    const apply = () => {
      ...
      ;(circleRef.current.material.uniforms as OrbUniforms).uInverted.value = isDark ? 1 : 0
    }
  ...
  useFrame((_, delta: number) => {
    ...
    const u = mat.uniforms as OrbUniforms
