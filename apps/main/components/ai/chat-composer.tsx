...
export const ChatComposer = forwardRef<ChatComposerHandle, {
    ...
    // ── Voice ──────────────────────────────────────────────────────────────
    // ElevenLabs Scribe, the same live transcription onboarding and the practice mentor
    // use. Speech lands in the field for review; nothing is sent until the user sends.
    const [mic, setMic] = useState<MicState>("idle")
    const [micNotice, setMicNotice] = useState<string | null>(null)
    // What was in the field when recording started, so a partial transcript replaces
    // only what the mic added.
    const baseRef = useRef("")
    const valueRef = useRef(value)
    valueRef.current = value
    const scribe = useScribe({
        onPartialTranscript: (data) => onChange(`${baseRef.current}${data.text}`),
    ...
    useEffect(() => { if (!scribe.isConnected && mic === "listening") setMic("idle") }, [scribe.isConnected, mic])
    ...
    const toggleMic = async () => {
        ...
        try {
            ...
            await scribe.connect({ token: token.token, microphone: { echoCancellation: true, noiseSuppression: true } })
            setMic("listening")
        } catch {
    ...
    const canSend = (value.trim().length > 0 || docs.length > 0) && uploading === 0 && !isStreaming
    const submit = () => {
        if (!canSend) return
        stopMic()
        onSubmit()
    ...
    const hint =
        mic === "listening" ? "Listening. Press Stop when you are done, then review and send."
            : mic === "connecting" ? "Starting the microphone…"
                : micNotice ?? "Enter to send · Shift+Enter for a new line · drop a file to attach"
        ...
        <div className={cn("shrink-0 px-3 pb-3 pt-2", wide && "mx-auto w-full max-w-3xl")}>
            ...
            <div className="rounded-2xl border border-neutral-200 bg-neutral-50 transition-colors focus-within:border-neutral-400 dark:border-neutral-700 dark:bg-neutral-900 dark:focus-within:border-neutral-500">
                ...
                <textarea
                    ...
                    placeholder={mic === "listening" ? "Listening…" : "Ask anything, or start something"}
                    aria-label="Message ShipItHQ AI"
