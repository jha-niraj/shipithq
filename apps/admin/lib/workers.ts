...
// ─────────────────────────────────────────────────────────────────────────────
// Dispatching a job to apps/worker from the admin console (plan/hiring-rounds
// HR-6: the company scrape).
//
...
function jobToken(userId: string, jobId: string): string {
    ...
    if (!secret) throw new Error("WORKER_SECRET is not set for the admin app")
