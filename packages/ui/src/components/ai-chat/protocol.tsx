...
/**
 * Something the assistant proposes to do, for a confirm card (plan/hiring-app
 * HA-12). The route saves it on the assistant turn; confirming acts on that
 * saved copy on the server.
 */
export interface ProposalFrame {
    t: "proposal";
    proposal: { kind: string; status: "pending" | "done" | "cancelled"; data: Record<string, unknown>; result?: { summary: string; href?: string } };
}

export type ChatFrame = TextFrame | ToolFrame | ActionFrame | DoneFrame | ErrorFrame | SessionFrame | TitleFrame | ProposalFrame;
